...
namespace Meter
{
    ...
    //                 {
    //                     ReferenceObject ro = Main.instance.references.references.Values.AsParallel().Where(n => n.codPlan == int.Parse(pv.cod)).FirstOrDefault();
    //                     if (ro != null)
    //                     {
    //                         ro.WriteToDB("план", "утвержденный", int.Parse(pv.day), pv.value.Replace(",","."));
    //                     }
    //                     else
    //                     {
