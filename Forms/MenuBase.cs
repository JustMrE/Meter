...
namespace Meter
{
    public partial class MenuBase : Form
    {
        ...
        protected virtual void Menu_Load(object sender, EventArgs e)
        {
            //listBox1.Items.AddRange(Main.instance.references.references.OrderBy(m => m._name).Select(n => n._name).ToArray());
        ...
        protected virtual void Menu_Close(object sender, EventArgs e)
        {
        ...
        protected virtual void listBox1_DoubleClick(object sender, EventArgs e)
        {
            ...
            //Marshal.ReleaseComObject(((Excel.Worksheet)Main.instance.xlApp.ActiveSheet).CodeName);
        }
        ...
        private void DontEdit(Excel.Range rng, int? day = null)
        {
            /*if (day != null)
            {
                string activeL1 = RangeReferences.activeTable.PS._activeChild._name;
                string activeL2 = RangeReferences.activeTable.PS._activeChild._activeChild._name;
                string adr = RangeReferences.activeTable.DB.childs[activeL1].childs["основное"].RangeByDay((int)day).Address[false,false];
                rng.Formula = @"=" + RangeReferences.activeTable.DB.WS.Name + "!" + adr;
            }*/
            //else
                rng.Formula = oldVal;
            //Marshal.ReleaseComObject(rng);
        ...
        protected void EmcosSelect()
        {
            using (EmcosPicker emcosPicker = new EmcosPicker())
            {
                ...
                {
                    //MessageBox.Show(emcosPicker.id.ToString() + " " + emcosPicker.ML_ID);
