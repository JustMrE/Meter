namespace Meter.Forms
{
    partial class NewMenuAdmin
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        protected void InitializeComponent()
        {
            this.SuspendLayout();
            //
            //flowlayoyt1
            //
            this.flpButtons.BackColor = Color.LightCyan;
            // 
            // NewMenuAdmin
            // 
            this.AutoScaleDimensions = new System.Drawing.SizeF(7F, 15F);
            this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
            this.BackColor = System.Drawing.SystemColors.GradientInactiveCaption;
            this.ClientSize = new System.Drawing.Size(1379, 230);
            this.Name = "NewMenuAdmin";
            this.Text = "NewMenuAdmin";
            this.ResumeLayout(false);
            //
            //ColorSettings (button37)
            //
            this.button37.Visible = true;
            this.button37.Text = "ResumeAll";
            //
            //ColorSettings (button38)
            //
            this.button38.Visible = true;
            this.button38.Text = "Новый лист";
            //
            //ColorSettings (button39)
            //
            this.button39.Visible = true;
            this.button39.Text = "Открыть логи";
            //
            //ColorSettings (button40)
            //
            this.button40.Visible = true;
            this.button40.Text = "Очистить логи";
            //
            //ColorSettings (button41)
            //
            this.button41.Visible = true;
            this.button41.Text = "Добавить субъект";
            //
            //ColorSettings (button42)
            //
            this.button42.Visible = true;
            this.button42.Text = "Цвета";
            
        }

        #endregion
    }
}